...
# region Prompts
def generate_base_system_prompt(user_plan: NutritionPlanModel):
    prompt = f"""
...
def handle_user_input(user_input: str) -> str:
    ...
    messages = [
        {"role": "system", "content": generate_base_system_prompt(get_user_daily_plan(user_id="test", date=datetime.now()))},
...
def generate_log_entry(meal_id: str, notes: str = None, image_url: str = None) -> LogEntry:
    ...
    return LogEntry(
        ...
        micronutrients=[_format_estimate(m) for m in response_content_json["micronutrients"]],
        notes=notes,
    )
...
if __name__ == "__main__":
    handle_user_input("Can I eat a banana?")